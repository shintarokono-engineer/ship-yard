...
### 関連する周辺整理(§9.12.3、2026-05-29 追加)

ADR-014 の `ANNOUNCEMENT_GEN` 導入で、既存 `DRAFT_GEN` が単一 DocType で生成していた `RELEASE_BLOG` / `TWEET` / `PRODUCT_HUNT` / `EMAIL` は配信用文面として `ANNOUNCEMENT_GEN` に役割が完全統合される。重複機能を抱えたままにしないため、`DocType` enum をユーザー判断で `README` / `OTHER` の **2 値に縮小** し DRAFT_GEN を README 専用に絞る(§9.12.1 LP 削除と同じ「型レベルから構造的に除去」 パターン、PROJECT_STATUS §9.12.3 参照)。本 Spec の §2「既存 DRAFT_GEN との使い分け」 もこれに合わせて更新済。
...
- **AIUsage 集計**:Feature 別に分けることで「告知に使ったクレジット」 と「README に使ったクレジット」 を分離して可視化
