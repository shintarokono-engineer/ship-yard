...
<!--
公開前の差し替え箇所(Day 48 の素材が揃ってから埋める)
- {{PRODUCT_URL}}      : https://neorie.com
- {{DEMO_VIDEO_URL}}   : Loom のデモ動画 URL
- {{SCREENSHOT_*}}     : docs/screenshots/ 配下の画像(Zenn は /images/ に配置)
- {{X_HANDLE}}         : 告知に使う X アカウント
公開タイミング: 本番リリースと同日。導入と末尾の CTA は「本日公開しました」前提で書いてあります。
-->

個人開発で B2B SaaS を作りました。マルチテナント、Stripe のシート課金、RAG、AI 機能一式が入っています。本日公開したので、作る過程で悩んだ設計判断を 6 つ書き出します。

「作ってみた」ではなく、**判断の理由と、判断を間違えて実際に壊れた話**を中心にします。同じところで悩む人の時間が少しでも減れば嬉しいです。

## 何を作ったか

**Neorie**(ネオリエ)という SaaS です。個人開発者と 2〜10 人の小規模チームが、「アイデア → 設計 → 開発 → リリース → 初期ユーザー獲得」までを 1 か所で回すためのツールです。
...
`injectTenantId` は操作の種類で差し込み先を変えます。読み取り・更新・削除は `where` に、`create` は `data` に、`upsert` は `where` と `create` の両方に入れます。Prisma 6 では `extendedWhereUnique` が GA なので、`findUnique` の `where` にも非ユニークフィールドを足せます。
...
呼び出し側が明示した `tenantId` とリクエストコンテキストの `tenantId` が食い違ったら、黙って上書きせず例外にします。これは「バグ」か「テナント越境の試み」のどちらかなので、静かに正しく直してしまうと**間違ったコードが動き続けてしまう**からです。同じ値の明示は許容します(後述の path slug ルートと共存させるため)。

### 自動注入が効かない穴は 2 つある

Extension は万能ではありません。実際に運用してみて、穴は 2 か所でした。
...
- Notion AI / Slack AI / Copilot Chat も横断参照はしない。B2B のデファクトから外れる
...
6 つを振り返ると、共通しているのは次の点でした。

1. **人間の注意力に依存する設計をやめる。** テナント分離は Client Extension と ESLint ルールで、環境変数は判断フローで、構造的に守る
2. **AI 機能は「動いた」を信じない。** 出力の形が正しくても経路が違うことがある。経路自体を記録して検証する
3. **外部連携は冪等に寄せて、ズレは後で直す前提にする。** 完全同期を狙うとユーザー体験が壊れる
4. **実装できるかより、自分のプロダクトで効くかを先に見積もる。** RAG はここで作り直しになりました

個人開発だと「とりあえず動かす」で進めがちですが、**あとから構造を入れ直すのが高くつくもの**(テナント分離、課金、AI の原価計算)は最初に決めておくと、後半が楽になります。

## 作ったもの

冒頭で紹介した Neorie は本日公開しました。アイデア検証・プロダクト診断・README / LP 生成・告知文の生成までを 1 か所で回せます。

- サービス: {{PRODUCT_URL}}
- デモ動画: {{DEMO_VIDEO_URL}}

新規登録から 7 日間、Pro の全機能をクレジットカード登録なしで試せます。個人開発中のプロダクトがある方は、アイデア検証あたりから触ってみてもらえると嬉しいです。

質問や「ここもっと詳しく」があれば、コメントか {{X_HANDLE}} まで。
