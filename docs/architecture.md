...
> **実際に発生した。** 診断 / 検証は 88〜113 秒かかり、`maxDuration = 60`(Vercel Hobby の上限)と
...
ブラウザ → Vercel (Server Action) → App Runner (API) → Anthropic
             ↑ 関数実行時間の上限        ↑ リクエストタイムアウト
```

**現状どちらにも明示的な設定がありません**(`apps/web` に `maxDuration` の指定なし、API 側の Anthropic 呼び出しにも timeout 指定なし)。Vercel の関数実行上限はプラン依存で、**Hobby は Pro より短い**ため、Hobby 継続の方針(`infrastructure-cost.md` §3.2 C)と組み合わさるとここが最初に効いてきます。

> **`runbooks/production-cutover.md` Phase 11 で、診断・検証の実測時間を必ず計測してください。** 上限に近ければ、キュー導入を待たずに `maxDuration` の引き上げ、または「実行を投げて結果は後から取得する」形への変更を検討する必要があります。

### トリガー 2: メール一斉配信を実装するとき(F21)
...
| 観点         | Redis + BullMQ             | PostgreSQL ベース(pg-boss 等) |
| ------------ | -------------------------- | ----------------------------- |
| 新規契約     | Upstash が必要             | 不要                          |
| 追加コスト   | 月 $0〜10(無料枠あり)      | $0                            |
| スループット | 高い                       | 中程度(MVP 規模なら十分)      |
| ADR との整合 | ADR-001 / 011 の想定どおり | 新規 ADR が必要               |

MVP 規模なら後者で足りる可能性が高いため、**実際に必要になった時点で比較して決める**方針とします。
