...
**最終更新**: 2026-05-18
**現在のフェーズ**: Week 2 完了(Day 15 完了)。バックエンド側は Week 2 スコープを 100% 消化(Project / ChecklistItem / Document の CRUD、4 つの AI 機能、RAG)。ただし **Week 2 のスコープに含まれていたフロントエンド(学習ログ UI / AI 機能 UI / 検索・フィルタリング UI)が Day 8〜15 で一切着手されず、Week 3 以降に丸ごとシフト**。再発防止のため Week 3〜6 を Day 単位に固定割当に引き直し(§6 参照)、Day 38 公開を目標(短縮版で Day 36 まで前倒し可能)
...
- **Day 16: AI エラーハンドリング共通化 + systemPrompt 共通化**(リファクタ)
  - Anthropic / OpenAI SDK 例外を共通の例外型(`AIServiceException` 等)でラップし、5 機能(DRAFT_GEN / CHECKLIST_GEN / REFINE_DOC / TASK_SPLIT / EmbeddingService)で統一
  - `ai/prompts/persona.ts` に「あなたは個人開発者・小規模チームの…」の冒頭テンプレを切り出して 4 機能で共有
  - 詳細は下記「Week 3 → Day 16」を参照
...
| --- | --- | --- |
| 16 | AI エラー共通化 + systemPrompt 共通化(リファクタ) | `apps/api/src/ai/` に共通例外型と `prompts/persona.ts`、5 機能で例外型統一・persona テンプレ共有 |
