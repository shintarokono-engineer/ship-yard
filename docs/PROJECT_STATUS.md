...
**Day 35-39 実装(2026-05-22、IaC コード)**:Week 5 の AWS リソースを Terraform で連続実装(`apply` は AWS アカウント作成後にまとめて 1 回)。`infra/prod/` に追加 — `rds.tf`(Day 35:RDS PostgreSQL `db.t4g.micro` + サブネット/パラメータグループ、TLS 強制、マスターパスワードは Secrets Manager 管理)、`nat.tf`(Day 36:fck-nat の NAT インスタンス + Private Subnet の外向きルート)、`apprunner.tf`(Day 36:App Runner サービス〔`enable_apprunner_service` で作成可否を切替、既定 false〕+ VPC コネクタ + RDS シークレット読取ポリシー)、`route53.tf`(Day 37:Route53 ホストゾーン + App Runner カスタムドメイン関連付け)、`cicd.tf`(Day 38:GitHub OIDC プロバイダ + デプロイ用 IAM ロール)、`monitoring.tf`(Day 39:CloudWatch アラーム + SNS 通知 + AWS Budgets)。`.github/workflows/deploy.yml` も追加。`terraform fmt` / `validate` 通過。セルフレビュー(`/reviewing-own-changes`、セキュリティ重点)で 🟡2(App Runner イメージタグの初回適用手順を明確化・NAT AMI の override 変数追加)+ 🟢6(`engine_version` ドリフト対策・NAT に EIP / SSM 経路・OIDC `sub` を main 限定・Budget に FORECASTED・変数 `validation`)を反映。**apply 前提**:環境固有変数(`domain_name` / `budget_alert_email`)の指定、`apps/api/Dockerfile` 作成 + API イメージ push 後に `enable_apprunner_service=true`、fck-nat AMI の owner/name 確認、GitHub Secrets 設定。`architecture.md` の追従更新と VPC Flow Logs は引き続きフォローアップ。
