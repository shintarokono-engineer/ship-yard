...
**最終更新**: 2026-05-20
**現在のフェーズ**: Week 3 完了 → Week 4 開始(Day 26 完了、Day 27 着手前)。Day 16-17 + Day 18/19/24/25 並行 BE + **Day 26 RAG コールドスタート対策**(ADR-008 起草・改訂 + `SEED_PUBLIC` テナント migration + OSS README 6 件 seed コーパス投入 + license attribution 自動付与 + `RagSearchService.searchSimilar` 拡張で `WHERE "tenantId" IN (caller, SEED_PUBLIC)` + `isSeed` フラグ追加)を 2026-05-19 に完了(E2E 2 回:旧テンプレ → 新 OSS 比較で seed 差し替え効果を実証)。**ロードマップ引き直し(2026-05-19)**:§9.4(RAG_QA + DRAFT_GEN 4 種別、+2 Day)+ §9.5(LP ブロック化、+4.5 Day を 4 Day に詰める)+ Day 26 残課題(prompt 改修、+0.3 Day)を「1 Day = 1 day 分」 原則で正直に配分。**Day 27 = RAG_QA、Day 28 = DRAFT_GEN 4 種別 + AIUsage 集計 + prompt 改修、Day 29-32 = LP ブロック化 Phase 1+2、Day 33-38 = Week 5 本番化(+5 シフト)、Day 39-43 = Week 6 公開準備(+5 シフト)**。**新公開目標 = Day 43**(元 Day 38 から +5 Day 後ろ倒し)、短縮版 Day 41。**Day 23 FE(TASK_SPLIT + REFINE_DOC UI)を 2026-05-20 に完了**(`feature/web-frontend`、フロントは Day 18 → 22 → 23 と進行。Day 18 / Day 22 は main マージ済、Day 23 は未マージ)
...
- Day 25 残作業:`GET /workspaces/:slug/usage`(AIUsage 月次集計 API)、`PATCH /me/profile`(Clerk 委譲で BE 不要かも、要確認)
...
### 今日のフォーカス(Day 26 完了 → Day 27 着手前)

- **Day 26 完了(2026-05-19)**:RAG コールドスタート対策(ADR-008 起草 + 実装 + 改訂)
  - ADR-008「RAG コーパス戦略(seed テナント + 段階的拡張)」 起草 + 同日中に改訂:ターゲット規模(個人開発者・小規模チーム)では 1 テナント数十件で頭打ちになる構造問題を、運営キュレーション seed テナント(`SEED_PUBLIC`)で補う方針。クロステナント RAG の致命的リスク(データ漏洩・法務)は排除、ユーザーのプライベートデータは決して横断しない
  - migration `20260519160000_add_seed_public_tenant`:System User(`usr_seed_system`) + `SEED_PUBLIC` Tenant + Sample Templates Project(`prj_seed_templates`)を冪等 INSERT(`P1002` advisory lock は前回失敗の残骸接続が原因、`pg_terminate_backend` で解消後に再適用成功)
  - seed コーパス: **当初の運営自作テンプレ 5 件(README 3 + LP 2)を品質問題で OSS 実 README 6 件に差し替え**(Hono / Zod / Drizzle / Astro / tRPC / Trigger.dev、すべて MIT または Apache-2.0)。frontmatter に `source_url` / `license` / `original_author` を必須化(all-or-nothing)、本文末尾に attribution ブロックを CLI が自動付与。差し替え理由は旧テンプレの「(プロジェクト名)」「(機能 1)」 等のプレースホルダが生成物に伝染することを E2E で確認したため。LP の seed は OSS に classic LP が存在しないため Day 26 では除外、Week 7+ で別途検討
  - CLI `pnpm --filter @shipyard/api seed-corpus:apply`(`scripts/seed-corpus.ts`):Markdown 読み込み → frontmatter パース(type / title 必須 + attribution 3 項目 all-or-nothing バリデーション)→ 本文末尾に attribution ブロック付与 → 冪等 upsert(同 (projectId, type, title) スキップ + MAX(version)+1 採番)→ embedding 自動生成
  - `RagSearchService.searchSimilar` 拡張:`options.includeSeed?: boolean`(デフォルト true)、`WHERE "tenantId" IN (caller, SEED_PUBLIC)` 形式に変更、`RagSearchHit.isSeed: boolean` フラグ追加(フロントで透明性表示用)
  - E2E 2 回実施(旧 seed と新 seed の比較):新 seed では「(your-name)」 や絵文字癖の伝染が解消され、OSS 的な汎用形に変化(状態タグ追加 / Node `>=18` のコードフェンス / build/start 節)。AIUsage で DRAFT_GEN 2429 input tokens(旧 3579 から -1150)、コスト 3.1 円(旧 3.28 円から微減)。**残課題**:プレースホルダ自体は依然多発するが、これは seed 起因ではなく `draft-gen.service.ts:71` の system prompt 設計起因(「事実不明部分はプレースホルダを置く」を明示指示)。Day 27 以降の prompt 改修案件として切り出し
- **Day 27 着手前**(2026-05-19 ロードマップ引き直し済):
  - Day 27 = RAG_QA(§9.4、1 day)
  - Day 28 = DRAFT_GEN 4 種別追加 + AIUsage 集計 + draft-gen prompt 改修(§9.4 / Day 25 残 / Day 26 残、合計約 1.6 day を 1 Day 枠に詰める)
  - Day 29〜32 = LP ブロック化 Phase 1+2(§9.5、4.5 day を 4 Day に詰める)
  - Week 5 = Day 33〜38(本番化、6 営業日)
  - Week 6 = Day 39〜43(公開準備、5 営業日)
  - **新公開目標 = Day 43**(元 Day 38 から +5 Day、§9.4 + §9.5 + Day 26 残のスコープ加算による)
...
| 25 | フロント:設定(プロフィール / メンバー / プラン / 利用状況) + **並行 BE:メンバー管理 API** | フロント:`/w/{slug}/settings/*`、AIUsage の今月使用量表示(別セッション) / 並行 BE(✅ 完了 2026-05-19):`GET /workspaces/:slug/members`(全員)、`PATCH .../members/:userId`(ロール変更)、`DELETE .../members/:userId`(削除 + 自己退会)、認可マトリクスを `MembersService` に集約(自己ロール変更不可 / OWNER 不可 / ADMIN→ADMIN 不可 / 自己退会は OWNER 以外可)、E2E 全 12 シナリオパス。AIUsage 月次集計 API は Day 25 残作業 |
| ✅ 26 | RAG コールドスタート対策(seed テナント + RagSearchService 拡張、ADR-008 改訂版) | `SEED_PUBLIC` テナント migration + OSS README 6 件 seed コーパス投入(Hono / Zod / Drizzle / Astro / tRPC / Trigger.dev、MIT / Apache-2.0)+ `RagSearchService` 拡張(`includeSeed` オプション、`isSeed` フラグ、`WHERE "tenantId" IN (caller, SEED_PUBLIC)`)+ license attribution 自動付与(frontmatter + 本文 attribution の二重記録)。E2E 2 回実施(旧テンプレ → 新 OSS 比較)で seed 差し替え効果を実証。残課題:プレースホルダ多発は prompt 設計起因と判明、Day 27 へ |
| 27 | **RAG_QA**(§9.4、2026-05-19 で MVP 必須化、+1 Day 純増)| `POST /workspaces/:slug/projects/:id/ai/qa`(過去 ProjectDocument 質問応答、Sonnet 4、BE + FE)。Shipyard のメイン用途(壁打ち)を直接実現する機能 |
| 28 | **DRAFT_GEN 4 種別追加 + AIUsage 集計 + draft-gen prompt 改修**(§9.4 / Day 25 残 / Day 26 残、+1 Day 純増)| `DraftGenService.generate` を RELEASE_BLOG / TWEET / PRODUCT_HUNT / EMAIL に拡張(`GENERATABLE_DOC_TYPES` 更新 + `kindLabel` / `structureHint` の switch 化、BE + FE のセレクタ追加)/ `GET /workspaces/:slug/usage`(AIUsage 月次集計、Day 25 残)/ `draft-gen.service.ts:71` の prompt 改修(プレースホルダ生成密度を下げる、Day 26 残課題) |

### Week 4 末:LP ブロック化 SaaS 化(Day 29〜32、4 営業日、§9.5 で MVP 必須化、+4.5 Day 純増)

§9.5 で 2026-05-19 に MVP 必須化された LP ブロック化(構造化 JSON ブロック型 + アプリ内編集 + 公開 URL)を 4 Day に配分。

| Day | 内容 |
| --- | --- |
| 29 | LP ブロック化 Phase 1(1/3:JSON 化 + AI Tool Use 修正、`submit_landing_page` 追加 + ブロックスキーマ 5 種定義) |
| 30 | LP ブロック化 Phase 1(2/3:アプリ内プレビュー UI、`apps/web/src/components/lp-blocks/<Hero/Features/Stats/Testimonial/CTA>Block.tsx`) |
| 31 | LP ブロック化 Phase 1(3/3:編集 UI、各ブロックのテキストフィールド編集、並び替えは v2) |
| 32 | LP ブロック化 Phase 2(公開 URL `shipyard.app/p/{slug}/{projectId}` + Clerk 認証除外 + `publishedAt` 公開トグル + OG メタ、`generateMetadata`) |

並行作業:ADR-009「LP ブロック型アーキテクチャ」 を Day 29 開始時に起票。

### Week 5:本番化(Day 33〜38、6 営業日)

| Day | 内容 |
| --- | --- |
| 33 | AWS インフラ:VPC / Subnet / IAM / SG / ECR |
| 34 | AWS:Aurora(PostgreSQL + pgvector 拡張)+ ElastiCache |
| 35 | AWS:ECS Fargate(API / Web 両方)+ ALB + 環境変数 |
| 36 | ドメイン取得 / Route53 / ACM / 本番 Clerk・Stripe・Resend 連携 |
| 37 | GitHub Actions で main → ECS 自動デプロイ |
| 38 | 監視(Sentry + CloudWatch)+ 本番疎通テスト |

### Week 6:リリース(Day 39〜43、5 営業日)

| Day | 内容 |
| --- | --- |
| 39 | ランディングページ(`/` の差し替え + OG 画像) |
| 40 | README 強化 + デモ動画(Loom / kapwing) |
| 41 | Zenn 記事(マルチテナント + RAG + 設計判断の言語化) |
| 42 | Twitter / プロダクトハント告知準備 |
| 43 | **公開リリース** |

### 短縮版(v2 送りで Day 41 公開、-2 日)

以下を v2(公開後)送りすれば Day 41 で公開可能(計 -2 日)。COMPETITOR_RESEARCH は元のスケジュールに含まれていないため、削減対象には入らない(現状で既に Week 7 = v2 候補):

| 項目 | 元 Day | 削減 |
| --- | --- | --- |
| 設定画面の請求書履歴・プロフィール編集を最小化 | Day 25 を半日 | -0.5 日 |
| 監視は Sentry のみ(CloudWatch ダッシュボードは後) | Day 38 を半日 | -0.5 日 |
| Zenn 記事は公開後の振り返り記事に | Day 41 をスキップ | -1 日 |
| **合計** | | **-2 日(Day 43 → Day 41)** |

更に短縮したい場合の追加候補(現状は不採用、判断保留中):

- ProjectDocument version 履歴 UI を最新版のみに簡略化(Day 21 半日 / -0.5 日、ただし Day 21 既に完了済のため適用不可)
- LP は最小限 1 ページ + デモ動画は静的スクショで代用(Day 39〜40 を 1 日に / -1 日)
- AWS の GitHub Actions 自動デプロイを初回手動運用にする(Day 37 半日 / -0.5 日)
- LP ブロック化 Phase 2(公開 URL)を v2 送り(Day 32 をスキップ / -1.5 日、ただし§9.5 で MVP 必須と判断したため要再評価)

→ Day 21 を除く 3 項目を全部加えると追加 -3 日で **Day 38 公開**(ただし「初回の見栄え」「運用の自動化」「LP の SaaS 化」の質が下がる)
...
- **Day 27〜28(§9.4 加算分)**: 「1 Day = 1 day 分」 原則を守って 2 Day に分割配分。Day 27 = RAG_QA(1 day)、Day 28 = DRAFT_GEN 4 種別追加 + AIUsage 集計 + draft-gen prompt 改修(合計約 1.6 day をやや詰めて 1 Day 枠)
- **Day 29〜32(§9.5 加算分)**: LP ブロック化 Phase 1(3 day)+ Phase 2(1.5 day)= 4.5 day を 4 Day に詰めて配分
- **公開日 Day 43 を目標**(2026-05-19 のスコープ追加 §9.4 +2 Day + §9.5 +4.5 Day + Day 26 残 +0.3 Day = 計 +5 Day シフト、元 Day 38 → Day 43)。短縮版 Day 41 公開(-2 Day)
- **削減候補(短縮版で取りうるもの)**:Week 5 監視ダッシュボード(Day 38 半日)、Week 6 Zenn 記事(Day 41 1 日)、LP ブロック化 Phase 2 を v2 送り(Day 32 / -1.5 日、ただし要再評価) → 必要に応じて Week 4 末に判断
...
- §9.4 加算(+2 Day)+ §9.5 加算(+4.5 Day を 4 Day に詰める)+ Day 26 残(+0.3 Day)= 計 +5 Day 後ろ倒し、**公開目標 Day 38 → Day 43 に変更**
- 短縮版で Phase 2 を v2 送りにする選択肢あり(Day 32 / -1.5 日、§6 短縮版「追加候補」 参照)、Week 4 末に再評価

**ADR 起票候補**:

- `docs/adr/009-landing-page-block-architecture.md`(LP ブロック型アーキテクチャの設計判断、ADR-005 を発展させる位置付け)
