...
| 35 | AWS:RDS PostgreSQL db.t4g.micro(pgvector)|
| 36 | AWS:NAT インスタンス + App Runner(API)+ VPC コネクタ。Web は Vercel |
| 37 | ドメイン取得 / Route53 / ACM / 本番 Clerk・Stripe・Resend・Upstash 連携 |
| 38 | GitHub Actions で main → App Runner / Terraform 自動デプロイ |
| 39 | 監視(Sentry + CloudWatch)+ AWS Budgets アラート + 本番疎通テスト |
