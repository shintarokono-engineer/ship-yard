...
**最終更新**: 2026-05-19
**現在のフェーズ**: Week 3 進行中。Day 16-17 + Day 18/19/24/25 並行 BE 作業を 2026-05-19 に完了(招待周辺 API 拡張 / テナント作成 API / メンバー管理 API / Stripe Portal Session API)。**Day 26(RAG コールドスタート対策)が進行中**:ADR-008(RAG コーパス戦略)起草済、`SEED_PUBLIC` テナント migration ファイル作成済(`P1002` advisory lock で適用未完了)、CLI スクリプトと RagSearchService 拡張は未着手。**未完了 BE 洗い出し(2026-05-19)で MVP スコープに追加確定**:RAG_QA(過去 ProjectDocument 質問応答 API、1 Day)、DRAFT_GEN の 4 種別追加(RELEASE_BLOG / TWEET / PRODUCT_HUNT / EMAIL、1〜1.5 Day、現状は README / LANDING_PAGE のみ対応)。ロードマップ +2 Day 規模、Day 27 旧バッファを再割当することで吸収予定(公開日 Day 38 維持を目標、短縮版 Day 36 は再評価)
...
### 今日のフォーカス(Day 26 進行中 + 未完了 BE 洗い出し)

- **Day 26 進行中**:RAG コールドスタート対策(ADR-008 起草済、`SEED_PUBLIC` テナント migration 作成済、`P1002` advisory lock で適用未完了で停止中)
- **未完了 BE 洗い出し(2026-05-19)で MVP スコープに追加確定**:
  - **RAG_QA**(過去 ProjectDocument 質問応答 API、約 1 Day):`schema.prisma` の `Feature.RAG_QA` 定義はあるがロードマップ未記載だった機能。`POST /workspaces/:slug/projects/:id/ai/qa` の 1 エンドポイント(質問入力 → RagSearchService で過去 ProjectDocument 検索 → Sonnet 4 で回答生成 → AIUsage 二段記録)。既存基盤の組み合わせで実装小規模
  - **DRAFT_GEN の 4 種別追加**(RELEASE_BLOG / TWEET / PRODUCT_HUNT / EMAIL、約 1〜1.5 Day):`ai.constants.ts:37` の `GENERATABLE_DOC_TYPES` が `[README, LANDING_PAGE]` の 2 種類のみで、残 4 種類が未対応。`DraftGenService.generate()` の `kindLabel`/`structureHint` を 2 分岐 if-else から switch 化 + 各種別の構造ヒント定義が必要。「個人開発リリース支援」 の独自性(リリース時の告知に直結)を担保するため MVP 必須化と判断
- **ロードマップ +2 Day 規模**:Day 27 旧バッファ(1 Day)を「RAG_QA + DRAFT_GEN 4 種別追加 + AIUsage 集計 + 残バッファ」 に再割当することで吸収予定。公開日 Day 38 維持を目標、短縮版 Day 36 は再評価
- **Day 27 残スコープ精査**:
  - Day 25 残作業:`GET /workspaces/:slug/usage`(AIUsage 月次集計 API、小)、プロフィール編集(Clerk 委譲で BE 不要の可能性、要確認)
  - 潜在的未実装(フロント要件次第):Project の `createdBy` join(🔴 高)、Project 集計 API / name 検索 / 複数 status / ソート(🟡〜🟢)、ChecklistItem / ProjectDocument の `createdBy` 展開(🟡)
