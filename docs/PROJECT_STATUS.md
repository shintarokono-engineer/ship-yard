...
**最終更新**: 2026-05-19
**現在のフェーズ**: Week 3 進行中(Day 17 完了、Day 18 / Day 19 フロントは別セッションで進行中)。Day 16 で AI 例外/プロンプト共通化(リファクタ)、Day 17 でメール基盤(Resend + React Email + InvitationsService 作成・承諾)を完成、Day 18 **並行 BE 作業として招待周辺 API を拡張**(GET 詳細 / GET 一覧 / DELETE 取り消し / POST 再送)、Day 19 **並行 BE 作業として `POST /workspaces`(テナント作成 API)を新規実装**(slug 自動生成 + Stripe Customer + Subscription FREE 初期化、Day 18 フロントオンボーディングのブロッカー解消)を 2026-05-19 に完了。フロント側 Day 18-19 は別セッションで進行中。Day 38 公開目標(短縮版で Day 36 まで前倒し可能)
