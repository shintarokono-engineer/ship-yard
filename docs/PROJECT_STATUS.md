...
**最終更新**: 2026-05-20
**現在のフェーズ**: Week 3 完了 → Week 4 進行中(Day 27 BE 完了、Day 28 着手前)。Day 16-17 + Day 18/19/24/25 並行 BE + Day 26 RAG コールドスタート対策 + **Day 27 RAG_QA BE 永続化実装**(2026-05-20、ADR-005 改訂節「Day 27 改訂:RAG_QA を MVP 必須化 + 対話履歴方針確定」 + `RagQaSession` / `RagQaMessage` 2 model + migration `20260520005022_add_rag_qa_session_and_message` + `RagQaService`(Sonnet 4 + 直近 N=10 ターン context + トランザクション同時保存)+ `RagQaController` 4 エンドポイント + E2E 9/9 ✅、7.61 円消費)。フロントは Day 18 → 22 → 23 と進行し、**Day 23 FE(TASK_SPLIT + REFINE_DOC UI)を 2026-05-20 に完了**(Day 18 / 22 / 23 すべて main マージ済)。**ロードマップ再引き直し(2026-05-20)**:RAG_QA を C 案永続化(`RagQaSession` + `RagQaMessage`)で実装し Day 27〜28 統合 → Day 29 以降を +1 Day シフト。**Day 28 = RAG_QA FE(セッション一覧 + チャット UI)、Day 29 = DRAFT_GEN 4 種別 + AIUsage 集計 + prompt 改修、Day 30-33 = LP ブロック化 Phase 1+2、Day 34-39 = Week 5 本番化(+1 シフト)、Day 40-44 = Week 6 公開準備(+1 シフト)**。**新公開目標 = Day 44**(元 Day 43 から +1 Day、案 1 採用)、短縮版 Day 42
...
### 今日のフォーカス(Day 27 BE 完了 → Day 28 着手前)
...
- **Day 28 着手前**(2026-05-20 ロードマップ再引き直し済、案 1 採用):
  - Day 28 = RAG_QA FE(`/w/{slug}/projects/{id}/qa` セッション一覧 + `.../qa/{sessionId}` チャット UI + 参照ドキュメント表示 + `isSeed` バッジ + サイドバー導線 + VIEWER 認可実機確認)
  - Day 29 = DRAFT_GEN 4 種別追加 + AIUsage 集計 + draft-gen prompt 改修(§9.4 / Day 25 残 / Day 26 残、合計約 1.6 day を 1 Day 枠に詰める)
  - Day 30〜33 = LP ブロック化 Phase 1+2(§9.5、4.5 day を 4 Day に詰める)
  - Week 5 = Day 34〜39(本番化、6 営業日)
  - Week 6 = Day 40〜44(公開準備、5 営業日)
  - **新公開目標 = Day 44**(元 Day 43 から +1 Day、RAG_QA C 案永続化採用による Day 27〜28 統合)、短縮版 Day 42
...
| 28 | **RAG_QA FE**(C 案永続化セッション UI、+1 Day 純増) | `/w/{slug}/projects/{id}/qa`(セッション一覧、ProjectsSidebar に導線)+ `.../qa/{sessionId}`(チャット UI、参照ドキュメント表示 + `isSeed` バッジ、送信中ローディング)+ VIEWER 認可実機確認(POST 不可 / GET 可) |
...
| Day | 内容 |
| --- | --- |
| 30 | LP ブロック化 Phase 1(1/3:JSON 化 + AI Tool Use 修正、`submit_landing_page` 追加 + ブロックスキーマ 5 種定義) |
| 31 | LP ブロック化 Phase 1(2/3:アプリ内プレビュー UI、`apps/web/src/components/lp-blocks/<Hero/Features/Stats/Testimonial/CTA>Block.tsx`) |
| 32 | LP ブロック化 Phase 1(3/3:編集 UI、各ブロックのテキストフィールド編集、並び替えは v2) |
| 33 | LP ブロック化 Phase 2(公開 URL `shipyard.app/p/{slug}/{projectId}` + Clerk 認証除外 + `publishedAt` 公開トグル + OG メタ、`generateMetadata`) |

並行作業:ADR-009「LP ブロック型アーキテクチャ」 を Day 30 開始時に起票。
