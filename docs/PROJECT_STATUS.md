...
**最終更新**: 2026-05-14
**現在のフェーズ**: Week 2 進行中(Day 12 完了)。OpenAI text-embedding-3-small で `ProjectDocument.embedding` を埋める基盤が整い、Day 13 の RAG 検索が着手可能
...
- **RAG 検索 + AI 生成への context 注入**(ADR-005 の独自性コア): `embedding <=> queryVec` で類似ドキュメント検索 → DRAFT_GEN / CHECKLIST_GEN のプロンプトに「参考: ...」として注入。コールドスタート対策(§9.3)も検討
- 残りの AI 機能(競合調査 / タスク分解 / 推敲)— `TASK_SPLIT` は Haiku 4.5 + Tool Use、`COMPETITOR_RESEARCH` は Sonnet 4 + Web Search Tool、推敲(REFINE_DOC)は Sonnet 4 で append-only に新版作成(Day 10 の edit 仕組みに乗せる)
- AI エラーハンドリングの共通化(Anthropic / OpenAI SDK 例外を `InternalServerErrorException` でラップ、DRAFT_GEN / CHECKLIST_GEN / EmbeddingService まとめて)
