...
**最終更新**: 2026-05-15
**現在のフェーズ**: Week 2 進行中(Day 14 完了)。RAG 検索を DRAFT_GEN / CHECKLIST_GEN / REFINE_DOC に注入する独自性コアが揃い、Day 15 以降は競合調査(Web Search Tool)とタスク分解、AI エラーハンドリングの共通化に着手可能
...
- **TASK_SPLIT**(Haiku 4.5 + Tool Use): 1 つの大きなタスクを ChecklistItem の配列に分解(データモデル上の入力表現を要設計)
- **COMPETITOR_RESEARCH**(Sonnet 4 + Web Search Tool): 競合調査、結果の保存先(ProjectDocument の新タイプ追加 or 別 entity)を要設計
- **AI エラーハンドリングの共通化**: Anthropic / OpenAI SDK 例外を `InternalServerErrorException` でラップ、DRAFT_GEN / CHECKLIST_GEN / REFINE_DOC / EmbeddingService まとめて
