...
**最終更新**: 2026-05-19
**現在のフェーズ**: Week 3 完了 → Week 4 開始(Day 26 完了、Day 27 着手前)。Day 16-17 + Day 18/19/24/25 並行 BE + **Day 26 RAG コールドスタート対策**(ADR-008 起草 + `SEED_PUBLIC` テナント migration + 5 件 seed コーパス投入 + `RagSearchService.searchSimilar` 拡張で `WHERE "tenantId" IN (caller, SEED_PUBLIC)` + `isSeed` フラグ追加)を 2026-05-19 に完了。E2E でテナント内 0 件状態でも seed が context に乗ることを実 Sonnet 4 で実証(input 3579 tokens のうち ~2000 が seed 由来)。**Day 27 着手前の確定スコープ**:RAG_QA(過去 ProjectDocument 質問応答 API、1 Day)、DRAFT_GEN 4 種別追加(RELEASE_BLOG / TWEET / PRODUCT_HUNT / EMAIL、1〜1.5 Day)、AIUsage 月次集計(Day 25 残、0.5 Day)。Day 27 旧バッファに再割当で吸収予定、公開日 Day 38 維持を目標
...
- **Day 27 着手前**:RAG_QA + DRAFT_GEN 4 種別追加 + AIUsage 集計(Day 25 残) + 残バッファ(§6 Week 4 表参照)+ DRAFT_GEN プロンプト改修(プレースホルダ生成ロジックの再設計、Day 26 残課題)
...
| 27 | **RAG_QA + DRAFT_GEN 4 種別追加 + AIUsage 集計 + 残バッファ**(2026-05-19 洗い出しで追加スコープ確定) | `POST /workspaces/:slug/projects/:id/ai/qa`(過去 ProjectDocument 質問応答、Sonnet 4)/ `DraftGenService.generate` を RELEASE_BLOG / TWEET / PRODUCT_HUNT / EMAIL に拡張(`GENERATABLE_DOC_TYPES` 更新 + `kindLabel` / `structureHint` の switch 化)/ `GET /workspaces/:slug/usage`(AIUsage 月次集計、Day 25 残)/ 残バッファ(積み残し回収) |
...
- **Day 27**: 旧バッファ(1 Day)を「**RAG_QA + DRAFT_GEN 4 種別追加 + AIUsage 集計(Day 25 残) + 残バッファ**」 に再割当することで、想定追加スコープ +2 Day 規模を吸収予定
  - 想定内訳:RAG_QA ≈ 1 Day(既存基盤組み合わせ)、DRAFT_GEN 4 種別 ≈ 1〜1.5 Day(`kindLabel` / `structureHint` の switch 化 + 各種別の構造ヒント設計)、AIUsage 集計 ≈ 0.5 Day
- **公開日 Day 38 維持を目標**。短縮版 Day 36 公開は実質困難になる可能性高、Week 4 進行中に Day 27 の作業量が見えた段階で再評価
- **削減候補(短縮版で取りうるもの)**:Week 5 監視ダッシュボード(Day 33 半日)、Week 6 Zenn 記事(Day 36 1 日)、ProjectDocument version 履歴 UI 簡略化(Day 21 半日) → 必要に応じて Week 4 末に判断
...
- 工数 4.5 日を Day 26-27(現状: RAG コールドスタート + バッファ + §9.4 で再割当済)へさらに織り込むか、独立 Day(Day 26.5 等)を追加するかを別途検討
- §9.4 で Day 27 に既に「RAG_QA + DRAFT_GEN 4 種別 + AIUsage 集計」を詰めているため、LP 4.5 日を加えると Day 38 公開予定が圧迫される可能性高
- Week 4 末に「LP MVP 着地 vs Day 38 公開維持」のトレードオフを再評価
