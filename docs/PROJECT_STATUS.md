...
**最終更新**: 2026-05-18
**現在のフェーズ**: Week 2 進行中(Day 15 完了)。TASK_SPLIT(Haiku 4.5 + Tool Use)+ ChecklistItem の self-relation(`parentId`、ON DELETE CASCADE)で「親タスク → サブタスク」の階層関係を構造的に保持。Day 16 以降は競合調査(Web Search Tool)、AI エラーハンドリングの共通化、RAG コールドスタート対策に着手可能
...
- **COMPETITOR_RESEARCH**(Sonnet 4 + Web Search Tool): 競合調査、結果の保存先(ProjectDocument の新タイプ追加 or 別 entity)を要設計
- **AI エラーハンドリングの共通化**: Anthropic / OpenAI SDK 例外を `InternalServerErrorException` でラップ、DRAFT_GEN / CHECKLIST_GEN / REFINE_DOC / TASK_SPLIT / EmbeddingService まとめて
- **RAG コールドスタート対策**(§9.3): オンボーディングでサンプル README/LP を seed 投入、初回から RAG が動く体験へ
- **AI 機能の systemPrompt 共通化**(セルフレビューでの低優先メモ): 4 機能(DRAFT_GEN / CHECKLIST_GEN / REFINE_DOC / TASK_SPLIT)で「あなたは個人開発者・小規模チームの…」の冒頭テンプレが重複 → `ai/prompts/persona.ts` に切り出すと、トーン変更時に 1 箇所で済む
