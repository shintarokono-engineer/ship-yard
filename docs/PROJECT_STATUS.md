...
**最終更新**: 2026-05-20
**現在のフェーズ**: Week 3 完了 → Week 4 進行中(Day 28 完了、Day 29 着手前)。Day 16-17 + Day 18/19/24/25 並行 BE + Day 26 RAG コールドスタート対策 + **Day 27 RAG_QA BE 永続化 + Day 28 RAG_QA FE**(2026-05-20、ADR-005 改訂節「RAG_QA を MVP 必須化 + 対話履歴方針確定」 + `RagQaSession` / `RagQaMessage` 2 model + `RagQaService` / `RagQaController` 4 エンドポイント + E2E 9/9 ✅、FE は セッション一覧 `/rag-qa` + チャット UI(`useOptimistic`)+ `RagQaMessage.references` 永続化(B 案))。フロントは Day 18 → 22 → 23 → 28 と進行(すべて main マージ済)。**ロードマップ(2026-05-20 再引き直し)**:RAG_QA を C 案永続化で実装し Day 27〜28 統合 → Day 29 以降を +1 Day シフト。**Day 29 = DRAFT_GEN 4 種別 + AIUsage 集計 + prompt 改修、Day 30-33 = LP ブロック化 Phase 1+2、Day 34-39 = Week 5 本番化、Day 40-44 = Week 6 公開準備**。**新公開目標 = Day 44**(元 Day 43 から +1 Day、案 1 採用)、短縮版 Day 42
