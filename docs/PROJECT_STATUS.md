...
**最終更新**: 2026-05-19
**現在のフェーズ**: Week 3 進行中(Day 17 完了、Day 18 フロントは別セッションで進行中)。Day 16 で AI 例外/プロンプト共通化(リファクタ)、Day 17 でメール基盤(Resend + React Email + InvitationsService 作成・承諾)を完成、Day 18 **並行 BE 作業として招待周辺 API を拡張**(GET 詳細 / GET 一覧 / DELETE 取り消し / POST 再送、`revokedAt` 列追加、計算プロパティ `InvitationStatus`)を 2026-05-19 に完了。フロント側 Day 18 オンボーディング UI は別セッションで進行中。Day 38 公開目標(短縮版で Day 36 まで前倒し可能)
...
- **Day 18 フロント(別セッションで進行中): オンボーディング(テナント作成 / メンバー招待 UI)**
  - サインアップ直後の `/onboarding` フロー(初回テナント作成、招待リンク承諾画面)
  - Day 17 / Day 18 並行 BE で実装済の招待 API を呼ぶフロント実装
  - shadcn/ui ベースのフォーム + バリデーション
- **Day 18 並行 BE 作業(2026-05-19 完了): 招待周辺 API 拡張**
  - `GET /invitations/:token`(未認証可、招待詳細表示。`PublicInvitationsController` 分離)
