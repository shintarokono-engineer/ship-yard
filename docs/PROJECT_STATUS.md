...
**最終更新**: 2026-05-12
**現在のフェーズ**: Week 1 / Day 6 完了、Day 7 開始前
...
| Day 7 | AI 基盤 + バッファ                  | ⬜ 次   | Anthropic API、pgvector、最初の AI 機能                                                                                                                                                                                         |

### Day 7 詳細(次にやること)

- `apps/api` に Anthropic SDK 統合(`@anthropic-ai/sdk`)+ `ANTHROPIC_API_KEY` の env 整備
- `AIUsage` テーブルへのテナント単位記録(モデル ID / feature / tokensIn・Out / costJpy)
- Free プランの月 20 回上限チェック(`AIUsage` の当月件数で判定、ADR-005)
- 最初の AI 機能:README / LP 自動生成(`DRAFT_GEN`、Sonnet 4 + Tool Use で構造化出力)
- pgvector への embedding 挿入(`ProjectDocument.embedding`、text-embedding-3-small 1536 次元、HNSW)— RAG の土台
- メール送信基盤の ADR-006(§9.2)を起こす候補 — Resend を MVP に採用する方向
- 注: AI 呼び出しは必ず `AIUsage` に記録してから / と同時にレスポンスを返す(課金・上限判定の根拠になるため取りこぼし禁止)
