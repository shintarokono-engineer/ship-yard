...
**最終更新**: 2026-05-13
**現在のフェーズ**: Week 2 進行中(Day 9 完了)。embedding 挿入は OpenAI キー未用意のため保留中
...
- ProjectDocument の編集・削除・推敲(version インクリメント)
- AI 機能の本実装(競合調査・タスク分解・チェックリスト生成・推敲)— `CHECKLIST_GEN` / `TASK_SPLIT` は Haiku 4.5 + Tool Use
