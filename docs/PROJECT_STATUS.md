...
**最終更新**: 2026-05-11
**現在のフェーズ**: Week 1 / Day 5 完了、Day 6 開始前
...
| Day 6 | Stripe 基盤                         | ⬜ 次   | Webhook 受信、商品セットアップ                                                                                                                                        |
| Day 7 | AI 基盤 + バッファ                  | ⬜      | Anthropic API、pgvector、最初の AI 機能                                                                                                                               |

### Day 6 詳細(次にやること)

- `apps/api` に Stripe SDK 統合 + テスト用 Product / Price セットアップ(Stripe Dashboard)
- `POST /webhooks/stripe` Endpoint(raw body での署名検証 = `stripe-signature` ヘッダー、ミドルウェア順序に注意)
- Webhook Idempotency 実装(`WebhookEvent.stripeEventId` ユニーク制約で重複処理を構造的に防止、ADR-004)
- `Subscription` model の DB 同期(`checkout.session.completed` / `customer.subscription.updated` / `customer.subscription.deleted` / `invoice.paid` / `invoice.payment_failed` の 5 イベント)
- Checkout Session 作成 API(後の Day 8〜で UI 連携)
- 注: ローカルテストは `stripe-cli` の Webhook forwarding を使う。Webhook 受信は `TenantMiddleware` が `tenantId` を確立できない例外ルートなので、`Subscription` の `tenantId` は Stripe metadata から解決する設計にする
