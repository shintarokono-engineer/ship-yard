...
**最終更新**: 2026-05-08
**現在のフェーズ**: Week 1 / Day 4 完了、Day 5 開始前
...
| Day 5 | マルチテナント基盤(NestJS + Prisma) | ⬜ 次   | NestJS 雛形、Prisma Client Extension(tenantId 自動注入)、AsyncLocalStorage、TenantContext                                       |
| Day 6 | Stripe 基盤                         | ⬜      | Webhook 受信、商品セットアップ                                                                                                  |
| Day 7 | AI 基盤 + バッファ                  | ⬜      | Anthropic API、pgvector、最初の AI 機能                                                                                         |

### Day 5 詳細(次にやること)

- `apps/api` に NestJS 雛形導入(TypeScript strict、`tsconfig.base.json` を継承)
- `packages/db` に Prisma schema(`docs/data-model.md` から実装)+ Client + マイグレーション初期化
- Prisma Client Extension で `tenantId` 自動注入(ADR-002)
- AsyncLocalStorage によるリクエストコンテキスト伝搬(`apps/web` の middleware が伝搬する `X-Tenant-Slug` を `apps/api` 側で受け取り、`tenantId` を解決)
- ESLint カスタムルール `no-raw-sql-without-tenant-filter` 雛形(ADR-002)
- Prisma ER 図自動生成(prisma-erd-generator)
- Clerk JWT を NestJS 側で検証する Guard 実装
- 「所属していない slug は 404(存在の有無を漏らさない)」を `/w/[slug]/page.tsx` で実装(Day 4 の `// TODO` を消化)
...
### 9.2 ADR-006 候補: メール送信基盤の選定(Day 6 / 7 で確定)

- `screen-flow.md` フロー 3(メンバー招待)に「SES 経由」の記述があるが、ADR・PROJECT_STATUS には未確定
- 候補:
  - **Resend**: 個人開発向け、API シンプル、無料枠 100 通/日、React Email 統合。3 週間 MVP に最適
  - **Postmark**: 配送性能トップクラス、有料($15/月〜)、トランザクショナルメール特化
  - **AWS SES**: 最安($0.10 / 1000 通)、本番スケール時の本命だが、初期は Sandbox モードで承認済みアドレスのみ送信可、解除申請が必要 → MVP 摩擦大
- 推奨方針: MVP は Resend、本番運用が安定したら SES への移行を検討
- 対応: Day 6 か Day 7 で ADR-006 を起こす
