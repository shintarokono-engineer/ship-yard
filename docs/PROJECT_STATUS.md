...
### Week 6:リリース(Day 40〜49、10 営業日、§9.9 PRODUCT_DIAGNOSIS の MVP 必須化で +5 Day)

| Day | 内容 |
| --- | --- |
| ✅ 40 | マーケティングランディングページ(`/` の差し替え + OG 画像) |
| ✅ 41 | README 強化(現状最新化、デモ動画 / スクリーンショット欄は Placeholder) |
| 42 | デモ動画(Loom)+ スクリーンショット 6 枚 + README の Placeholder 差し替え |
| 43 | **PRODUCT_DIAGNOSIS**:ADR-012 起票 + schema(`ServiceScore` model + migration)+ 設計確定(§9.9) |
| 44-45 | **PRODUCT_DIAGNOSIS BE 実装**(`ProductDiagnosisService` + Sonnet 4 + Web Search Tool + Tool Use `submit_service_score` + AIUsage 統合 + `Feature.PRODUCT_DIAGNOSIS` 追加 + E2E) |
| 46-47 | **PRODUCT_DIAGNOSIS FE 実装**(診断トリガー UI + 結果表示 + 履歴比較 + Pro/Team プランゲート + セルフレビュー) |
...
- LP は最小限 1 ページ + デモ動画は静的スクショで代用(Day 40〜41 を 1 日に / -1 日、ただし Day 40-41 既に完了済のため適用不可)
...
- PRODUCT_DIAGNOSIS の Web Search Tool 連携を v1.x 送り(Day 44-45 のうち 1 日削減 / -1 日、ただし§9.9 で「競合データ込みでこそ差別化機能」と判断したため要再評価)

→ Day 21 / Day 40-41 を除く 3 項目を全部加えると追加 -2.5 日で **Day 44 〜 45 公開**(ただし「運用の自動化」「LP の SaaS 化」「診断機能の競合データ」 の質が下がる)
...
**ロードマップ調整(2026-05-25 確定)**:

- 工数 5 Day を Week 6 に挟む形で配分:Day 43 = 設計 + ADR-012 + schema、Day 44-45 = BE 実装(Sonnet 4 + Web Search Tool + Tool Use + E2E)、Day 46-47 = FE 実装(診断 UI + 履歴比較 + プランゲート + セルフレビュー)、Day 48 = Zenn 記事 + 告知準備、Day 49 = 公開
- **公開目標 Day 44 → Day 49** に +5 Day シフト(§6 Week 6 表 / §7 公開目標を同時更新)
- 短縮版は Day 47 公開(Zenn 記事を公開後送りで -2 Day)、`PRODUCT_DIAGNOSIS の Web Search Tool 連携を v1.x 送り` の追加削減候補もあるが §9.9 で「競合データ込みでこそ差別化機能」 と判断したため要再評価
