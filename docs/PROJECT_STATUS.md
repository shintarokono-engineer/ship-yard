...
**最終更新**: 2026-05-18
**現在のフェーズ**: Week 3 進行中(Day 16 完了、Day 17 着手準備)。Day 8〜15 でバックエンド完成 → Day 16 で AI 例外型統一(`AIBadResponseError` = 502)+ systemPrompt 共通化(`AI_PERSONA_INTRO` / `taskItemGuidance`)のリファクタを完了。残作業は Week 3 の Day 17〜21(メール基盤 + フロント基盤)、Week 4 の AI/Stripe フロント、Week 5 の本番化、Week 6 の公開準備。Day 38 公開目標(短縮版で Day 36 まで前倒し可能)
...
- **Day 17: メール基盤(Resend + MailService 抽象 + 招待トークン送信)**
  - ADR-007 で MVP 採用と決まっている Resend を実装する。`apps/api/src/mail/` に `MailService` 薄抽象を置き、プロバイダ差し替えを構造的に可能にする
  - `InvitationToken` を使った招待 API(`POST /workspaces/:slug/invitations` 作成、`POST /invitations/:token/accept` 承諾)を完成させ、招待メールを送る
  - 本番 DNS(SPF / DKIM / DMARC)は Week 5 の本番化で対応、Day 17 では Resend の `onboarding@resend.dev` 等のテスト用送信アドレスでローカル疎通まで
  - 詳細は下記「Week 3 → Day 17」を参照
...
| 17 | メール基盤(Resend + MailService 抽象 + 招待トークン送信) | `apps/api/src/mail/`、ADR-007 の実装、`InvitationToken` を使った招待 API 完成 |
