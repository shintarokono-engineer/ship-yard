...
- ADR-005 で RAG を独自性のコアと位置付けたが、初回利用者は過去 `AIDocument` が 0 件 → RAG が空振りするコールドスタート問題がある
- そのまま実装すると、新規ユーザーが「ただの Sonnet 4 生成」しか体験できず差別化が伝わらない
- 対策案(Week 2 で設計に組み込む):
  - オンボーディングで「サンプル README / LP テンプレート」を seed として `AIDocument` に投入し、初回から RAG を動かす
