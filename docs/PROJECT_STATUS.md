...
- **Day 29 着手前**:
  ...
  - Week 5 = Day 34〜39(本番化)、Week 6 = Day 40〜49(公開準備 + §9.9 PRODUCT_DIAGNOSIS)
  - **公開目標 = Day 49**(2026-05-25 §9.9 で +5 Day 後ろ倒し、元 Day 44)、短縮版 Day 47
...
### Week 6:リリース(Day 40〜49、10 営業日、§9.9 PRODUCT_DIAGNOSIS の MVP 必須化で +5 Day。2026-05-25 に開発タスクを前倒しする再配分を実施)

| Day | 内容 |
| --- | --- |
| ✅ 40 | マーケティングランディングページ(`/` の差し替え + OG 画像) |
| ✅ 41 | README 強化(現状最新化、デモ動画 / スクリーンショット欄は Placeholder) |
| 42 | **PRODUCT_DIAGNOSIS**:ADR-012 起票 + schema(`ServiceScore` model + migration)+ 設計確定(§9.9) |
| 43-44 | **PRODUCT_DIAGNOSIS BE 実装**(`ProductDiagnosisService` + Sonnet 4 + Web Search Tool + Tool Use `submit_service_score` + AIUsage 統合 + `Feature.PRODUCT_DIAGNOSIS` 追加 + E2E) |
| 45-46 | **PRODUCT_DIAGNOSIS FE 実装**(診断トリガー UI + 結果表示 + 履歴比較 + Pro/Team プランゲート + セルフレビュー) |
| 47 | デモ動画(Loom)+ スクリーンショット 6 枚 + README の Placeholder 差し替え(PRODUCT_DIAGNOSIS 完成版を含む) |
| 48 | Zenn 記事(マルチテナント + RAG + PRODUCT_DIAGNOSIS の設計判断)+ Twitter / Product Hunt 告知準備 |
| 49 | **公開リリース** |
...
- **公開日 Day 49 を目標**(2026-05-25 §9.9 で PRODUCT_DIAGNOSIS を MVP 必須化、+5 Day で Day 44 → Day 49)。短縮版 Day 47 公開(-2 Day)
...
### 9.8 Pro / Team プランのカニバリゼーション(プラン構造の見直し)

2026-05-22、運用コストの議論からプラン設計の論点が浮上。ADR-004 の現行プラン表は Free(¥0、メンバー 3 人まで、AI 20 回)/ Pro(¥980 定額、メンバー無制限)/ Team(¥2,800/人、+ 共同編集・レビュー・監査ログ)。

**問題**: Pro が「メンバー無制限」かつ協業機能なしのため、複数人チームが Pro ¥980 定額で済ませられ、Team(人数課金)が売れない(カニバリゼーション)。Free も 3 人まで許容しており「Free 3 人 > Pro」というメンバー数の不整合もある。協業機能を持たない Free / Pro が複数メンバーを持てる意味が曖昧。

**推奨案(2026-05-22)**: 「人数でプランを分ける」。Free = 1 人 / Pro = 1 人 / Team = 2 人以上。メンバー招待・6 ロール・共同編集・レビュー・監査ログを丸ごと Team 専用にし、Free / Pro は個人専用にする。境界が「1 人 = Free/Pro、2 人以上 = Team」と明快になりカニバリが構造的に消える。CLAUDE.md のターゲット定義(個人開発者 / 小規模チーム)とも一致。無料で小チームを試したい層は Team の期間制トライアルでカバー。

**対応**: ADR-004 は承認済みのため、確定時は ADR-012(または ADR-004 改訂)を起こす。プラン確定は公開前(Week 6 の課金まわり着手時)を目処とする。データモデル変更は不要で、`Tenant.plan` によるプラン制限判定に「Free / Pro はメンバー 1 人」を追加する程度。
...
- 工数 5 Day を Week 6 に挟む形で配分。**2026-05-25 中に開発タスクを前倒しする再配分を実施**:Day 42 = 設計 + ADR-012 + schema、Day 43-44 = BE 実装(Sonnet 4 + Web Search Tool + Tool Use + E2E)、Day 45-46 = FE 実装(診断 UI + 履歴比較 + プランゲート + セルフレビュー)、Day 47 = デモ動画 + スクショ + README 差し替え(PRODUCT_DIAGNOSIS 完成版を含む)、Day 48 = Zenn 記事 + 告知準備、Day 49 = 公開
...
**ADR-012(起票予定、Day 43)**: 「PRODUCT_DIAGNOSIS:プロダクト診断機能のアーキテクチャ」 として、Sonnet 4 + Web Search Tool + Tool Use の組み合わせ採用理由、rubric 設計、`ServiceScore` 履歴モデル、Pro/Team プランゲートの判断、LLM スコアの限界と緩和策、競合データ取得時の rate limit / コスト管理を起こす。
