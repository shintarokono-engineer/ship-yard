...
**最終更新**: 2026-05-14
**現在のフェーズ**: Week 2 進行中(Day 11 完了)。embedding 挿入は OpenAI キー未用意のため保留中
...
- 残りの AI 機能(競合調査 / タスク分解 / 推敲)— `TASK_SPLIT` は Haiku 4.5 + Tool Use、`COMPETITOR_RESEARCH` は Sonnet 4 + Web Search Tool、推敲(REFINE_DOC)は Sonnet 4 で append-only に新版作成
- RAG(過去 `ProjectDocument` のベクトル検索 → 生成時に context 注入、ADR-005 の独自性コア。コールドスタート対策は §9.3)
- AI エラーハンドリングの共通化(Anthropic SDK 例外を `InternalServerErrorException` でラップ、DRAFT_GEN / CHECKLIST_GEN まとめて)
