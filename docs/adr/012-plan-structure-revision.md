...
> **2026-08-23 追記(F20)**: 実際のトライアル期間は「7 日 + その日の残り時間」になっています。当初の `trial_period_days: 7` はテナント作成時刻に終了時刻が依存するため、12:00 JST 固定で走るバッチから「今日で終了するテナント」を判定できませんでした。終了時刻を JST の日末(23:59:59)に揃えた明示的な `trial_end` を渡す方式に変更し解消しましたが、代わりにユーザー間で最大 24 時間の trial 期間差が生まれます。コストは 300cr の AI クレジット上限で頭打ちになるため許容しています。
...
- ~~**トライアル終了通知メール**(3 日前、当日)~~ → **✅ 2026-08-23 実装完了**(F20)。EventBridge Rule + API destination による日次バッチ。設計は [`../superpowers/specs/2026-08-23-f20-trial-reminder-email-design.md`](../superpowers/specs/2026-08-23-f20-trial-reminder-email-design.md)
