...
承認済み(2026-05-01)。プラン構造の表(月額・メンバー・AI 回数・機能)は **ADR-012 で部分改訂**(2026-05-25)。Stripe 連携方式・Subscription Quantity・Webhook Idempotency 等の仕組みは本 ADR を継続する。

## 背景・問題

Neorie の収益モデルとプラン構造を確定し、Stripe との連携設計を行う必要がある。MVP リリース時から課金可能な状態を目指す。Free プランでユーザー獲得、有料プランで個人開発者・チームを収益化する設計が必要。
...
> ⚠️ **2026-05-25 ADR-012 で改訂**:下表は**初期案(2026-05-01 時点)**で、現行プランは [ADR-012「決定」節](./012-plan-structure-revision.md#決定)を参照。改訂内容は (1) Free 廃止 → 新規登録は 7 日 Pro トライアル、(2) Pro ¥980 → ¥1,480、(3) AI 無制限 → クレジット制(Pro 300 / Team 800 cr/人)、(4) Team は 2 人以上専用化。下表は履歴として残す。
...
- `Tenant.plan` — `Subscription.plan` を**ホットパス用にコピー**したもの。「このワークスペースのプランは?」はプラン制限の判定(Free → AI 機能停止、Pro/Team → AI クレジット上限判定、Team → 招待・監査ログ表示 等、ADR-012)で多用され、`Tenant` はテナント解決のためほぼ毎リクエスト読まれる。`Tenant` 側に `plan` があれば `Subscription` への JOIN を毎回しなくて済む
