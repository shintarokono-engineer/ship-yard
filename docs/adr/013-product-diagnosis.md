...
既に作ったプロダクトを採点するより、**発案時点(IDEA 段階)で競合調査をして方向修正する** のが Lean Startup の Problem-Solution Fit 検証の流れ。「事後の評価」 ではなく「事前の方向修正」 こそが Shipyard の差別化価値の中核。本 ADR 当初設計はこの「アイデア検証フェーズ」 を取りこぼしていた。
...
- 「発案 → 実装 → 改善」 のループ全てを Shipyard で完結させる訴求が公開時の差別化として最大
