...
- Anthropic の Web Search Tool は本機能で初導入(現状の Neorie では未使用)
...
- 短所:
  ...
  - 「競合調査だけ」 単体機能としての価値が薄い(Neorie はプロダクトを作るユーザー向けで、競合調査は手段であって目的ではない)

### C. 完全分離(スコア化は競合データ不要)

- 概要:`PRODUCT_DIAGNOSIS` は Neorie 内部データ(ProjectDocument / ChecklistItem / LandingPage)のみで動く。`COMPETITOR_RESEARCH` は独立機能として残す(v2 候補のまま)
- 長所:
...
- 短所:
  ...
  - Neorie の差別化価値が弱まる(「AI で診断」 は他社 SaaS でもあるが、「Web Search で実競合と比較して診断」 は差別化要素)
...
改善提案セクションには「選んでタスク化」 ボタンを置く(**F17、2026-08-30 実装完了**)。カードごとではなくセクションに 1 つ置き、選んだ提案をまとめて 1 回の AI 呼び出しで処理する。**1 対 1 の転記ではなく、AI(Haiku 1cr)が実行可能なタスクへ分解する**形を採った。提案の `body` は Markdown の箇条書きを許容していて 1 提案に複数作業が並ぶうえ、`ChecklistItem.category` は提案の `axis` からは決まらない(別次元の分類)ため、転記だと 1 提案が技術とマーケに跨るとき片方しか選べないため。
...
- **Neorie の差別化価値**:Web Search 込みの実競合比較が他社 AI アシスタント(Cursor / Cline / Continue 等)との差別化要素になる。「AI で診断」 は普及しているが「実競合と Web 比較して診断」 は新しい価値
...
- **B**:競合調査単体機能の需要が低い(Neorie ユーザーは「プロダクトを作る個人開発者・小規模チーム」 で、競合調査は副次目的)。UX が冗長で MVP の差別化価値が弱まる
- **C**:「競合優位性」 軸が空想ベースになりスコアの信頼性が下がる。Neorie の差別化要素が「単なる AI スコア化」 に矮小化される
...
- 公開時の Hero / Features に「**実競合と Web 比較して 100 点満点で診断する Pro/Team 限定機能**」 を訴求でき、Neorie の差別化が立つ(§9.9)
...
- 履歴比較で「自分のプロダクトの実用性スコアが時系列で上がっていく」 が見える化され、Neorie の継続利用動機になる
- 改善提案を ChecklistItem に変換する導線(**F17、2026-08-30 実装完了**)により、診断 → 改善 → 再診断のループが Neorie 内で完結する
...
## 改訂(2026-05-25):2 モード化 — アイデア検証機能を別機能として追加

Day 43 BE 骨組み実装後のユーザーレビューで、本 ADR 当初設計に 2 つの本質的な問題が判明したため、PRODUCT_DIAGNOSIS を **2 つの独立機能** に分離する改訂を行う。

### 改訂の背景

#### 問題 1:データ不足時の診断品質

本 ADR 当初設計は `Project` の `IN_DEV` 以降のデータ(README / LP / ChecklistItem)を全て読んで採点する前提だが、これらが空 or 薄い段階(IDEA 状態)で診断すると AI は「機能完成度 0 点 / リリース準備度 0 点」 と冷たい採点になり、スコアが極端に低くなる。「自分のプロダクトはまだダメ」 という主観だけが残り、ユーザーへの示唆が出ない。

#### 問題 2:競合調査のタイミング

既に作ったプロダクトを採点するより、**発案時点(IDEA 段階)で競合調査をして方向修正する** のが Lean Startup の Problem-Solution Fit 検証の流れ。「事後の評価」 ではなく「事前の方向修正」 こそが Neorie の差別化価値の中核。本 ADR 当初設計はこの「アイデア検証フェーズ」 を取りこぼしていた。

### 改訂内容:機能分離(2 つの独立した API)

PRODUCT_DIAGNOSIS を 2 機能に分離する(同一エンドポイントでのモード分岐ではなく、UI / API レベルで機能を分ける = ユーザー視点で「アイデア検証」 と「プロダクト診断」 は別物として明示)。

| 機能                                                                                | 対象 status                                 | 評価軸                                                                                                                     | 出力                                                                                                              | API パス                                                                         | DB テーブル                 |
| ----------------------------------------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | --------------------------- |
| **アイデア検証**(IdeaValidation、**新規**)                                          | `IDEA`                                      | 5 軸 × 各 20 点(`problemClarity` / `targetClarity` / `differentiation` / `competitiveAdvantage` / `marketPotential`)       | 総合スコア(0-100)+ ブレークダウン + **`recommendation: GO \| PIVOT \| NO_GO`**(意思決定支援)+ 改善提案 + 競合参照 | `POST /workspaces/:slug/projects/:projectId/idea-validations`(+ GET 一覧 / 詳細) | 新規 `IdeaValidation` model |
| **プロダクト診断**(ServiceScore、**本 ADR 当初定義のまま、Day 43 BE 骨組み実装済**) | `IN_DEV` / `BETA` / `LAUNCHED` / `ARCHIVED` | 5 軸 × 各 20 点(`differentiation` / `targetClarity` / `featureCompleteness` / `releaseReadiness` / `competitiveAdvantage`) | 総合スコア(0-100)+ ブレークダウン + 改善提案 + 競合参照                                                           | `POST /workspaces/:slug/projects/:projectId/diagnoses`(+ GET 一覧 / 詳細)        | 既存 `ServiceScore` model   |

UI 配置:Project 詳細画面で `status` を見て、IDEA 時は **「アイデア検証」 Card のみ**、IN_DEV 以降は **「プロダクト診断」 Card のみ** を表示(両方は同時に出さない、明示的な機能切替)。

### Project model に詳細情報フィールド追加(両機能の入力源)

診断時にフォーム入力させるのではなく、**プロジェクト編集画面の「詳細情報」 タブで一度入力** → AI 機能側は保存済データを読むだけ、という UX 設計。

| フィールド         | 型                   | 用途                                     |
| ------------------ | -------------------- | ---------------------------------------- |
| `targetUsers`      | `String?` `@db.Text` | 想定ユーザー(誰の課題か)                 |
| `problemStatement` | `String?` `@db.Text` | 解きたい課題                             |
| `proposedFeatures` | `String?` `@db.Text` | 想定機能リスト(Markdown)                 |
| `pricingModel`     | `String?`            | 想定価格モデル(例「Free + Pro ¥980/月」) |

- IDEA 時はこれらの入力をユーザーに強く促す(空だとアイデア検証ボタンを無効化、または警告)
- IN_DEV 以降でもプロダクト診断の「targetClarity」「differentiation」 軸の評価精度が上がる(README に書かれていない事業情報の補完)

### `IdeaValidation` model
...
model IdeaValidation {
  ...
  tenantId        String
  projectId       String
  totalScore      Int      // 0-100、breakdown 合計と整合性アサート
  recommendation  String   // 'GO' | 'PIVOT' | 'NO_GO'(LLM が rubric に従って判定)
  breakdown       Json     // Record<IdeaValidationAxis, { score, comment }>(5 軸)
  suggestions     Json     // [{ priority, title, body, axis }]
  competitorRefs  Json     // [{ name, url, summary, similarityNote }](ServiceScore.competitorRefs と同型)
  webSearchUsed   Boolean
  modelUsed       String
  createdById     String
  createdAt       DateTime @default(now())
  ...
  @@index([tenantId])
  @@index([projectId, createdAt])  // 履歴比較の高速取得
}
```

`recommendation` は Lean Startup の意思決定支援として明示。スコアだけ見せても「次に何をすべきか」 が伝わらないため、`GO`(進める)/ `PIVOT`(方向修正)/ `NO_GO`(断念検討)の 3 値判定を併記。

### Feature enum 追加

`Feature.IDEA_VALIDATION` を新規追加(`PRODUCT_DIAGNOSIS` と並ぶ、`ALTER TYPE ... ADD VALUE` 単独 migration)。AIUsage の集計でアイデア検証とプロダクト診断を区別する。

### MVP 採用の判断

ユーザー判断で **MVP に含める**(v1.x 送りではなく、公開時に両機能を揃える)。理由:

- 「発案 → 実装 → 改善」 のループ全てを Neorie で完結させる訴求が公開時の差別化として最大
- LP の Hero / Features に「**アイデア段階から実競合と比較して Go/Pivot/No-Go を判定**」 を加えられる(Pro プランの目玉機能 2 つになる)
- Zenn 記事のメインテーマを「アイデア検証 + プロダクト診断 = Lean Startup 全段階を AI 支援」 として書ける

### 公開スケジュールへの影響

**公開目標 Day 49 → Day 52 に +3 Day シフト**:

| Day   | 内容                                                                                                                  |
| ----- | --------------------------------------------------------------------------------------------------------------------- |
| ✅ 43 | プロダクト診断 BE 骨組み(本 ADR 当初定義、完了)                                                                       |
| 44    | アイデア検証 BE(schema + service + controller + dto)+ Project 詳細フィールド追加 + プロダクト診断 runDiagnosis 本実装 |
| 45    | プロダクト診断 + アイデア検証の E2E + AIUsageService.assertWithinDiagnosisQuota                                       |
| 46-47 | FE:プロジェクト編集の「詳細情報」 タブ + プロダクト診断 UI + アイデア検証 UI                                          |
| 48    | FE 仕上げ + セルフレビュー + デモ動画 + スクショ                                                                      |
| 49-50 | README 差し替え + Zenn 記事 + 告知準備                                                                                |
| 51-52 | 公開リリース                                                                                                          |

短縮版:Day 49 公開(アイデア検証を v1.x 送り)も可能。本改訂では MVP 維持の方針。

### v1.x 候補(改訂で生まれた新フォローアップ)

- アイデア検証から IN_DEV 遷移時のスコア継承(IDEA → IN_DEV 切替時に IdeaValidation の breakdown を「初期値」 として ServiceScore に引き継ぎ)
- 「Pivot 提案」 を ChecklistItem に変換する導線(アイデア検証の suggestions に基づく)
- アイデア検証の「市場性」 軸を強化するため、トレンド検索(Google Trends API 等)の統合検討

### 改訂による既存 ServiceScore への影響
...
直前の改訂(2026-05-26)で導入した 5 構造化セレクト(`targetUserAttrs` / `problemCategory` / `coreFeatures` / `pricingType` / `pricingRange`)を、コミット前のセルフレビューで Neorie ユーザーから以下の根本的な設計問題が指摘された:

- **ユーザー職種(ENGINEER / DESIGNER / MANAGER ...)が B2B SaaS 前提**: Neorie 自体が B2B SaaS なので発想がそこに引っ張られたが、Neorie ユーザーが作るプロダクトは B2C(ゲーム / SNS / 一般消費者向けツール)も含み多様。
...
根本的な問題は**「構造化セレクトの選択肢が B2B SaaS 前提の語彙で作られている」**こと。Neorie ユーザーが作るプロダクトは多様で、コア機能・課題カテゴリのような網羅必須項目をセレクトで表現するのは現実的でない。
