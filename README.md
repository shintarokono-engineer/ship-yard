...
> **現在のステータス**: Week 2 進行中(Day 9 完了)。`apps/web`(Next.js 15 + Tailwind v4 + shadcn/ui + Clerk)+ `apps/api`(NestJS 11)+ `packages/db`(Prisma 6 / PostgreSQL 16 + pgvector)が稼働。マルチテナント基盤(Prisma Client Extension で `tenantId` 自動注入、`TenantMiddleware`、Clerk JWT Guard)+ Stripe 課金基盤(Webhook 署名検証 + Idempotency、`BillingService` で Subscription / `Tenant.plan` 同期、Checkout)+ AI 基盤(Anthropic SDK、`AIUsage` 記録 + Free 月 20 回上限、`POST .../documents/generate` で Sonnet 4 + Tool Use による README/LP 生成)+ Project / ChecklistItem の CRUD API・ProjectDocument 閲覧 API(`class-validator` でリクエスト検証、`@Roles` + `WorkspaceGuard` による RBAC、コントローラ/サービス層分離)実装済み・実 API で E2E 確認済み。次は Day 10〜(AI 機能本実装 / RAG / embedding 挿入)。
...
│   └── api/        @shipyard/api   - NestJS 11 + Prisma 統合 + TenantMiddleware + Clerk JWT Guard + Stripe(Webhook / Subscription 同期 / Checkout)+ AI(Anthropic SDK / AIUsage / README・LP 生成)+ Project / ChecklistItem / ProjectDocument の CRUD・閲覧 API(class-validator / RBAC ガード / コントローラ・サービス層分離)(Day 9 完了)
├── packages/
