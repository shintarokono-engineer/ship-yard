...
Neorie(ネオリー)は、個人開発者および小規模開発チーム(2〜10 人)向けの、「アイデア → 設計 → 開発 → リリース → 初期ユーザー獲得」までを一元管理する AI 支援付き B2B SaaS です。
...
![Neorie のデモ](./docs/screenshots/neorie-demo.gif)

アイデア検証 → プロダクト診断 → LP 生成 → 告知配信 の流れです(25 秒 / 音声なし / テロップ付き)。AI の実行待ちは省いています。
...
開発の過程で判断したことを Zenn に書いています。

| 記事                                                                                                                                                                 | 内容                                                                          |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| [個人開発で B2B SaaS を作るなら先に知りたかった、マルチテナント・RAG・AI 課金の設計判断 6 つ](https://zenn.dev/akutagawa/articles/neorie-multitenant-rag-ai-billing) | テナント分離の自動注入 / RAG のコールドスタート / AI クレジットと TOCTOU など |
| [個人開発の B2B SaaS で使う技術を、こう選んで、こう外した](https://zenn.dev/akutagawa/articles/neorie-tech-selection-and-rejection)                                  | 技術選定の判断軸と棄却理由、本番構築後の答え合わせ                            |
