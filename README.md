...
> **現在のステータス**: Week 2 進行中(Day 10 完了)。`apps/web`(Next.js 15 + Tailwind v4 + shadcn/ui + Clerk)+ `apps/api`(NestJS 11)+ `packages/db`(Prisma 6 / PostgreSQL 16 + pgvector)が稼働。マルチテナント基盤(Prisma Client Extension で `tenantId` 自動注入、`TenantMiddleware`、Clerk JWT Guard)+ Stripe 課金基盤(Webhook 署名検証 + Idempotency、`BillingService` で Subscription / `Tenant.plan` 同期、Checkout)+ AI 基盤(Anthropic SDK、`AIUsage` 記録 + Free 月 20 回上限、`POST .../documents/generate` で Sonnet 4 + Tool Use による README/LP 生成)+ Project / ChecklistItem の CRUD API・ProjectDocument の閲覧/編集(append-only)/削除(soft delete)API(`class-validator` でリクエスト検証、`@Roles` + `WorkspaceGuard` による RBAC、コントローラ/サービス層分離、partial index で生存行のみ index 化)実装済み・実 API で E2E 確認済み。次は Day 11〜(AI 機能本実装 / RAG / embedding 挿入)。
...
# 5. 環境変数を設定(Clerk / DB / Stripe / AI)
cp apps/web/.env.example  apps/web/.env.local   # Clerk publishable/secret key, API_URL
cp apps/api/.env.example  apps/api/.env.local   # DATABASE_URL, CLERK_SECRET_KEY, PORT, STRIPE_*, APP_BASE_URL, ANTHROPIC_API_KEY
...
#  - ANTHROPIC_API_KEY(https://console.anthropic.com)。AI 機能を動かさないならプレースホルダのままでも起動はする
...
│   └── api/        @shipyard/api   - NestJS 11 + Prisma 統合 + TenantMiddleware + Clerk JWT Guard + Stripe(Webhook / Subscription 同期 / Checkout)+ AI(Anthropic SDK / AIUsage / README・LP 生成)+ Project / ChecklistItem の CRUD API + ProjectDocument の閲覧/編集(append-only)/削除(soft delete)API(class-validator / RBAC ガード / コントローラ・サービス層分離 / partial index)(Day 10 完了)
├── packages/
