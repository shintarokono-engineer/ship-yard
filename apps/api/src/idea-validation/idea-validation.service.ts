...
export class IdeaValidationService {
  ...
  async startValidation(input: {
    ...
    if (running) {
      throw new ConflictException('このプロジェクトでは検証が実行中です。完了までお待ちください。');
    }
    ...
    //    検証は 88〜113 秒かかり Vercel Hobby の関数実行上限(60 秒)と FE の API_TIMEOUT_MS
    ...
    void this.executeValidation(input, reservationId, jobId);
    return { jobId };
  }
  ...
  private async executeValidation(
    ...
    try {
      ...
      await this.aiJobs.complete(jobId, validation.id);
    } catch (err) {
      ...
      const translated = translateAIProviderError(err, 'IDEA_VALIDATION', this.logger);
      await this.aiJobs.fail(
        jobId,
        translated instanceof Error
          ? translated.message
          : 'アイデア検証の実行に失敗しました。時間をおいて再実行してください。',
      );
    }
  }
}
