 ...
 *
 * 本文の「停止範囲」と「データ保持」の記述は、現時点の FREE フォールバックの実装
 * (`assertWithinPlanCredits` 等が AI 機能のみを止め、プロジェクト/ドキュメントの
 * 閲覧・編集は止めない、かつ削除フローが未実装)に基づく。ADR-004 の解約後削除フロー
 * (7 日 grace → 30 日凍結 → 削除)を実装する際は、この本文を必ず見直すこと。
 */
        ...
        <Container style={containerStyle}>
          ...
          <Section>
            ...
            <Text style={textStyle}>
              ...
              機能(ドキュメント生成・壁打ち・診断・告知文生成)が停止します。プロジェクトやドキュメントの閲覧・編集は、その後も引き続きご利用いただけます。
              <strong>
                作成済みのプロジェクトやドキュメントはそのまま残り、引き続き閲覧いただけます。
              </strong>
            </Text>
            <Text style={textStyle}>
