 ...
 *
 * - `ServiceScore`(プロダクト診断)には recommendation が無いので、IdeaValidation でのみ使用
 * - tone に応じて色を変える(positive=primary / neutral=secondary / negative=destructive)
 * - ラベル・説明文は `VALIDATION_RECOMMENDATION_META` を流用(BE/FE で文言一元化)
 * - `showDescription={false}` で Badge のみ(履歴一覧の行のように説明を出す余地が無い場所用)
 */
